...
class SerialPort:
    ...
    def _check_header_crc(self):
        ...
        crc = crc_func(bytes(self._rec_header,'utf-8') + self._rec_data_buf[0:self._header_crc_offset])
        if crc != header_crc:
            return False
        return True
    ...
    def serial_protocol_build(self, payload):
        packet = bytes(self._send_header,'ASCII')
        packet += len(payload).to_bytes(2, byteorder='big', signed=False)
        crc_func = crcmod.mkCrcFun(0x11021, rev=False, initCrc=0x0000, xorOut=0x0000)
        if type(payload) is str:
            crc = crc_func(bytes(payload, 'utf-8'))
        else:
            crc = crc_func(payload)
        packet += crc.to_bytes(2, byteorder='big', signed=False)
        crc_func = crcmod.mkCrcFun(0x11021, rev=False, initCrc=0x0000, xorOut=0x0000)
        crc = crc_func(packet)
        packet += crc.to_bytes(2, byteorder='big', signed=False)
        if type(payload) is str:
...
if __name__ == "__main__":
    ...
    # erase flash
    num_pages = math.ceil(total_bytes / 2048)
    packet = bytes('E', 'ASCII')
    packet += num_pages.to_bytes(1, byteorder='big', signed=False)
    ...
    total_packets = math.ceil(len(input_firmware) / bytes_per_packet)
    ...
    while programming:
        if send_packet:
            ...
            packet = bytes('F', 'ASCII')
            packet += current_packet.to_bytes(2, byteorder='big', signed=False)
            last_packet = 1
            if current_packet < total_packets - 1:
                last_packet = 0
            packet += last_packet.to_bytes(1, byteorder='big', signed=False)
